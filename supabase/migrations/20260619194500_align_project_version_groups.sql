-- `version_number` is the public V1/V2/V3 grouping key. Keep the legacy
-- `major_version` column aligned so web and desktop uploads cannot diverge.
UPDATE public.project_versions
SET major_version = version_number
WHERE major_version IS DISTINCT FROM version_number;
...
BEGIN
  ...
  SET version_number = next_version_number,
      major_version = next_version_number
  WHERE id = _version_id
