...
BEGIN
  ...
  -- For now restrict to self; future: public profile flag
  IF target_user <> auth.uid() THEN
    RAISE EXCEPTION 'Not authorized';
  END IF;

  -- Hero numbers (only versions uploaded BY this user)
  SELECT
    count(*),
  ...
  -- Collaborated projects (projects user contributed to but doesn't own)
  ...
  -- Avg versions per project (iteration depth) — among projects user touched
  ...
  -- Streaks — based on distinct local-day saves
  WITH days AS (
  ...
  -- BPM histogram (per project, only non-null bpm)
